...
const filterByCategory = (categoryId) => {
    router.get(route('admin.articles.index'), { category: categoryId || undefined }, { preserveState: true });
};
    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center justify-between">
                ...
                <Link :href="route('admin.articles.create')" class="inline-flex items-center gap-2 px-4 py-2 bg-primary-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-primary-700 transition">
        ...
        <div class="py-12">
            <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
                ...
                <div class="mb-6 flex items-center gap-3 flex-wrap">
                    ...
                    <button @click="filterByCategory(null)" :class="['px-3 py-1.5 text-sm rounded-full border transition', !filters?.category ? 'bg-primary-600 text-white border-primary-600' : 'bg-white text-gray-600 border-gray-300 hover:border-primary-300']">
                        Semua
                    </button>
                    <button v-for="cat in categories" :key="cat.id" @click="filterByCategory(cat.id)" :class="['px-3 py-1.5 text-sm rounded-full border transition', filters?.category == cat.id ? 'bg-primary-600 text-white border-primary-600' : 'bg-white text-gray-600 border-gray-300 hover:border-primary-300']">
                        {{ cat.name }}
                    </button>
                </div>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            <tr v-for="article in articles.data" :key="article.id" class="hover:bg-gray-50 transition">
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-4">
                                        <img v-if="article.image_url" :src="article.image_url" class="w-16 h-12 rounded-lg object-cover border border-gray-200" />
                                        <div v-else class="w-16 h-12 rounded-lg bg-gray-100 flex items-center justify-center text-gray-400 text-xs">No Img</div>
                                        <div>
                                            <div class="text-sm font-bold text-gray-900 line-clamp-1">{{ article.title }}</div>
                                            <div class="text-xs text-gray-500">oleh {{ article.user?.name || 'Unknown' }}</div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    <span v-if="article.category" class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-primary-50 text-primary-700">
                                        {{ article.category.name }}
                                    </span>
                                    <span v-else class="text-xs text-gray-400">Tanpa Kategori</span>
                                </td>
                                <td class="px-6 py-4">
                                    <span v-if="article.published_at" class="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-50 text-green-700">
                                        <Eye class="w-3.5 h-3.5" /> Published
                                    </span>
                                    <span v-else class="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-medium bg-yellow-50 text-yellow-700">
                            ...
                            <tr v-if="articles.data.length === 0">
                                <td colspan="5" class="px-6 py-12 text-center text-gray-500">
                                    Belum ada artikel. <Link :href="route('admin.articles.create')" class="text-primary-600 font-bold hover:underline">Tulis artikel pertama →</Link>
                                </td>
                            </tr>
                        </tbody>
                    </table>
