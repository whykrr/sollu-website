...
<template>
    ...
    <MainLayout>
        ...
        <div class="py-20 bg-gray-50">
            <div
                class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-32 relative z-20"
            >
                <div class="grid md:grid-cols-3 gap-6">
                    <div
                        v-for="(plan, index) in pageContents?.pricing_plans
                            ?.attributes?.plans || []"
                        :key="index"
                        :class="[
                    ...
                    >
                        ...
                        >
                            <span
                                class="bg-yellow-400 text-yellow-900 text-xs font-black uppercase tracking-wider py-1 px-3 rounded-full"
                                >Paling Laris</span
                            >
                        </div>
                        ...
                        <ul class="space-y-2 flex-grow mb-8">
                            <li
                ...
                <div class="mt-32 text-center">
                    ...
                    >
                        {{
                            pageContents?.trusted_by?.title ||
                            "Dipercaya oleh lebih dari 5.000 bisnis di Indonesia"
                        }}
                    </p>
                    <div
                        class="flex justify-center gap-12 flex-wrap opacity-50 grayscale"
                    >
                        <template
                            v-if="
                                pageContents?.trusted_by?.attributes
                                    ?.brand_logos?.length
                            "
                        >
                            <div
                                v-for="(logo, idx) in pageContents.trusted_by
                                    .attributes.brand_logos"
                                :key="idx"
                                class="h-8 md:h-10 text-xl font-bold text-gray-800 tracking-tighter"
                            >
                                {{ logo.name }}
                            </div>
                        </template>
